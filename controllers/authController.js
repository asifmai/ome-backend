...
exports.register_phone = async function(req, res, next) {
  try {
    const errors = [];
    const phone = req.body.phone ? formatter.formatPhone(req.body.phone) : undefined;
    ...
    if (errors.length > 0) {
      ...
      for (let i = 0; i < reimbursements.length; i++) {
        if (reimbursements[i].phone == newUser.phone) {
          await Reimbursement.findByIdAndUpdate(reimbursements[i]._id, {userId: newUser._id});
        }
      }
