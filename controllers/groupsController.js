    ...
    for (let i = 0; i < members.length; i++) {
      ...
      if (foundUser) {
        if (foundUser.phone == req.user.phone) {
          return res.status(422).json({ status: 422, msg: 'You cannot add yourself as a group member'});
        }
...
module.exports.groups_get = async (req, res) => {
  try {
    ...
    const groups = await Group.find({userId}).populate({
      path: 'members transactions',
      populate: {
        path: 'user',
        select: '-password -verificationCode -createdAt -updatedAt -salt -verified'
      },
    }).exec();
...
module.exports.update_group_post = async (req, res) => {
  try {
    ...
    for (let i = 0; i < members.length; i++) {
      members[i].user = null;
      const foundUser = await User.findOne({verified: true, phone: members[i].phone});
      if (foundUser) {
        if (foundUser.phone == req.user.phone) return res.status(422).json({ status: 422, msg: 'You cannot add yourself as a group member'});
        members[i].user = foundUser._id;
      } else {
        const smsBody = `OME\n${req.user.profile.firstName} has added you to a group. Please download OME from the link below to signup.`;
        twilio.sendSMS(members[i].phone, smsBody);
...
module.exports.add_transaction_post = async (req, res) => {
  try {
    ...
    if (!foundGroup) return res.status(422).json({ status: 422, msg: 'Group not found'});
    if (!foundTransaction) return res.status(422).json({ status: 422, msg: 'Transaction not found'});

    const transactions = foundGroup.transactions;
    if (transactions.some(tr => tr == transactionId)) return res.status(422).json({ status: 422, msg: 'Transaction is already added to the group'});
    ...
    res.status(200).json({status: 'success', msg: 'Transaction Added to Group successfully'});   
  } catch (error) {
